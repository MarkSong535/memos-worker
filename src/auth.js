...
export async function validateIdentity(token, key, config, clientId, nonce) {
  ...
  if (!role) throw new Error('Required group missing');
  return { payload, role };
}
async function discovery(env) {
  const url = env.OIDC_DISCOVERY_URL;
  if (!url || !env.OIDC_CLIENT_ID || !env.OIDC_CLIENT_SECRET || !env.APP_ORIGIN) throw new Error('SSO is not configured');
...
export async function authRoute(request, env) {
  ...
  try {
    ...
    return json({ error: 'SSO sign-in failed. Check configuration and membership in notes or notes_admin.' }, 403);
  }
}
